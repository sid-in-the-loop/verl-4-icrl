...
$$\max_{\theta,\phi} \mathbb{E}_{x \sim \mathcal{D}} \mathbb{E}_{c \sim \pi_\theta(\cdot \mid x)} \mathbb{E}_{y \sim p_\phi(\cdot \mid x,c)} \big[ R_{\text{task}}(x,y) \big]$$
...
5. GRPO objective:
   $$\max_\phi \mathbb{E}\left[\sum_i A_i \log p_\phi(y_i \mid x)\right] - \beta_{\text{LL}} \mathbb{E}\big[ D_{\text{KL}}(p_\phi \,\|\, p_{\text{ref}}) \big]$$
...
$$c_i \sim \pi_\theta(\cdot \mid x), \quad y_i \sim p_\phi(\cdot \mid x, c_i)$$
...
$$\max_\phi \mathbb{E}\left[\sum_i A_i \log p_\phi(y_i \mid x,c_i)\right] - \beta_{\text{LL}} \mathbb{E}\big[ D_{\text{KL}}(p_\phi(\cdot \mid x,c)\,\|\,p_{\text{ref}}(\cdot \mid x,c)) \big]$$
...
$$U_\phi(x,c) = \mathbb{E}_{y \sim p_\phi} \big[\log p_\phi(y \mid x,c) - \log q(y \mid x)\big]$$

To prevent pathological divergence:

$$\tilde{U}_\phi(x,c) = \min(U_\phi(x,c), \tau)$$

---

### 4.3 Final LL Objective

$$\max_\phi \mathbb{E}_{x,c,y} \big[ R_{\text{task}}(x,y) + \lambda \cdot \tilde{U}_\phi(x,c) \big]$$
...
$$r_i^{\text{total}} = R_{\text{task}}(x,y_i) + \lambda \cdot \text{clip}\big( \log p_\phi(y_i \mid x,c_i) - \log q(y_i \mid x), \tau' \big)$$
...
$$R_{\text{HL}}(x,c;\phi) = R_{\text{task}}(x,y) + \alpha \cdot H\big(p_\phi(\cdot \mid x,c)\big)$$

Equivalent confidence form:

$$R_{\text{HL}}(x,c;\phi) = R_{\text{task}}(x,y) - \alpha \cdot \mathbb{E}_{y \sim p_\phi}\big[\log p_\phi(y \mid x,c)\big]$$
...
$$\max_\theta \mathbb{E}_{x,c \sim \pi_\theta} \big[ R_{\text{HL}}(x,c;\phi) \big] - \beta_{\text{HL}} D_{\text{KL}}\big( \pi_\theta(\cdot \mid x)\,\|\,\pi_{\text{SFT}}(\cdot \mid x) \big)$$
...
$$\phi_t \leftarrow \arg\max_\phi \mathbb{E}\big[ R_{\text{task}} + \lambda \tilde{U}_\phi \big] \quad \text{via GRPO}$$

### Step 2: HL Update (Fix $\phi$)

$$\theta_t \leftarrow \arg\max_\theta \mathbb{E}\big[ R_{\text{HL}}(x,c;\phi_t) \big] - \beta_{\text{HL}} D_{\text{KL}}(\pi_\theta \| \pi_{\text{SFT}})$$
